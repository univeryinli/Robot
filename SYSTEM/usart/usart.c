...
void USART1_rx(void)
{
	...
	if(USART_RX_STA&0x8000)
		{					   
			len=USART_RX_STA&0x3fff;    //获取数据的长度
// 	  	printf("\r\n您发送的消息为:\r\n\r\n");
			for(t=0;t<len ;t++)
			{
				USART_SendData(USART1, USART_RX_BUF[t]);   //发送获取到的数据到串口，两个参数都为全局变量
				while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);  //等待发送完成，发送完成后才能循环到发送下个数据
			}
			check(len); 	
			printf("\r\n\r\n");
			USART_RX_STA=0;
